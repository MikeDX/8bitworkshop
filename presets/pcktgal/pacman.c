/*
 * Pac-Man on Pocket Gal (Data East BAC06 / 6502).
 * First bring-up from pacmanonpacman — screen crop TBD.
 */
...
/* HUD lives on logical row 2 (viewport top after LOGIC_Y_SHIFT). */
#define HUD_Y  2

/* Right-aligned score ending at (x,y); no leading zeros (min "00" = tens×10). */
static void draw_score_r(byte x, byte y, word n) {
  ...
  /* Draw over in place — field is blanked only on full HUD force. */
  pos = x;
  poke_tile(pos, y, (byte)'0', 0x0F);
  pos--;
  poke_tile(pos, y, (byte)('0' + d4), 0x0F);
  if (n >= 10) { pos--; poke_tile(pos, y, (byte)('0' + d3), 0x0F); }
  if (n >= 100) { pos--; poke_tile(pos, y, (byte)('0' + d2), 0x0F); }
  if (n >= 1000) { pos--; poke_tile(pos, y, (byte)('0' + d1), 0x0F); }
  if (n >= 10000) { pos--; poke_tile(pos, y, (byte)('0' + d0), 0x0F); }
}

/* Active-player "P1" blinks via color RAM only. */
...
static void flash_1up(void) {
  ...
  flash_1up_pal = pal;
  poke_pal(0, HUD_Y, pal);
  poke_pal(1, HUD_Y, pal);
}
...
void draw_hud(void) {
  ...
  /*
   * Single top line: P1 ####### HI #######
   * (font is uppercase-only, so "hi"→"HI"). P2 omitted (no 2P mode).
   */
  if (force) {
    for (i = 0; i < 28; i++)
      poke_tile(i, HUD_Y, T_BLANK, 0x0F);
    put_string(0, HUD_Y, "P1", 0x0F);
    put_string(10, HUD_Y, "hi", 0x0F);
    flash_1up_pal = 0x0F;
    draw_score_r(8, HUD_Y, score);
    draw_score_r(18, HUD_Y, hiscore);
    hud_score = score;
    hud_hiscore = hiscore;
    hud_ready = 1;
    return;
...
static void attract_draw_copyright(void) {
  ...
  put_string(6, 31, "1987 DX AUTOMATICS", 3);
