 ...
 * ~15/~14 = 65520/65521 as word — avoids cc65 "integer constant is long". */
static const word sc_bounds[3][7] = {
  { 420, 1620, 2040, 3240, 3540, 4740, 5040 },           /* L1: … C20 S5 */
  { 420, 1620, 2040, 3240, 3540, (word)~15, (word)~14 }, /* L2–4: C1033 S1f */
  { 300, 1500, 1800, 3000, 3300, (word)~15, (word)~14 }  /* L5+: C1037 S1f */
};
...
byte update_ghost_dir(byte i) {
  ...
  if (mode == MODE_ENTER) {
    if (g->tx == ANTE_TX) {
      /* ANTE_OX==0: any nonzero ox is to the right → always DIR_LEFT. */
      if (g->ox != ANTE_OX)
        g->next_dir = DIR_LEFT;
      else
        g->next_dir = DIR_DOWN;
